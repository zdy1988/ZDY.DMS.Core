define(["jquery", "ko", "app"], function ($, ko, app) {
    var _ = {};
    ...
    _.user = {
        set: function (value) {
            return $.Deferred(function (defer) {
                _.cookie("zdy_user", JSON.stringify(value), { expires: 1 });
                defer.resolve();
            }).promise();
        },
        get: function () {
            var user = _.cookie("zdy_user");
            if (user !== undefined) {
    ...
    _.token = {
        set: function (value) {
            return $.Deferred(function (defer) {
                _.cookie("zdy_token", value, { expires: 1 });
                defer.resolve();
            }).promise();
        },
        get: function () {
            return "Bearer " + _.cookie("zdy_token");
        }
    };
    ...
    _.alert = {
        error: function (text) {
            if (window.parent.alert) {
                return window.parent.alert.error(text);
            } else {
    ...
    _.toastr = {
        error: function (text) {
            if (window.parent.toastr) {
                window.parent.toastr.error(text);
            } else {
        ...
        warning: function (text) {
            if (window.parent.toastr) {
                window.parent.toastr.warning(text);
            } else {
        ...
        success: function (text) {
            if (window.parent.toastr) {
                window.parent.toastr.success(text);
            } else {
        ...
        info: function (text) {
            if (window.parent.toastr) {
                window.parent.toastr.info(text);
            } else {
    ...
    _.confirm = function (text) {
        if (window.parent.confirm) {
            return window.parent.confirm(text);
        } else {
            return app.sweetAlert({
                text: text,
