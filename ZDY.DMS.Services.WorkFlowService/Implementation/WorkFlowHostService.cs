...
namespace ZDY.DMS.Services.WorkFlowService.Implementation
{
    public class WorkFlowHostService : ServiceBase<WorkFlowServiceModule>, IWorkFlowHostService
    {
        ...
        /// <returns></returns>
        public async Task Execute(WorkFlowExecution execute)
        {
            //加载运行实例
        ...
        /// <returns></returns>
        private async Task<Tuple<WorkFlowTask, WorkFlowStep>> GetTaskAndStepForExecuting(WorkFlowExecution execute)
        {
            ...
            var currentStep = execute.WorkFlowDefinition.GetStep(execute.StepId);
        ...
        /// <param name="execute"></param>
        /// <returns></returns>
        private async Task GetExecuteHandler(WorkFlowExecution execute)
        {
            ...
            foreach (var item in execute.Steps)
            {
                ...
                var step = execute.WorkFlowDefinition.GetStep(item.Key);
        ...
        /// <returns></returns>
        private async Task<List<WorkFlowUser>> GetStepHandler(WorkFlowStep step, WorkFlowExecution execute, List<WorkFlowUser> appointHandlers = null)
        {
        ...
        /// <param name="step"></param>
        /// <returns></returns>
        private async Task<List<WorkFlowUser>> GetCopyToUsers(WorkFlowStep step, WorkFlowExecution execute)
        {
        ...
        /// <param name="execute"></param>
        /// <returns></returns>
        private async Task ExecuteSubmit(WorkFlowExecution execute, WorkFlowInstance instance, Tuple<WorkFlowTask, WorkFlowStep> current)
        {
        ...
        /// </summary>
        /// <param name="execute"></param>
        private async Task ExecuteBack(WorkFlowExecution execute, WorkFlowInstance instance, Tuple<WorkFlowTask, WorkFlowStep> current)
        {
            var currentTask = current.Item1;
            var currentStep = current.Item2;
        ...
        private async Task ExecuteRedirect(WorkFlowExecution execute, WorkFlowInstance instance, Tuple<WorkFlowTask, WorkFlowStep> current)
        {
            var currentTask = current.Item1;
            var currentStep = current.Item2;
        ...
        private void ExecuteWorkFlowControl(WorkFlowExecution execute, WorkFlowInstance instance, Tuple<WorkFlowTask, WorkFlowStep> current)
        {
            var currentStep = current.Item2;
            var currentNextSteps = execute.WorkFlowDefinition.GetNextSteps(current.Item2.StepId);
            ...
            if (currentNextSteps.Any(t => t.IsHandleBy(WorkFlowHandlerKinds.AnyUser)))
            {
                ...
                {
                    var s = execute.WorkFlowDefinition.GetStep(step.Key);
        ...
        /// 由系统控制时，检测步骤间的条件是否满足
        /// </summary>
        private void ExecuteConditionsBetweenSteps(WorkFlowExecution execute, WorkFlowInstance instance, Tuple<WorkFlowTask, WorkFlowStep> current)
        {
            ...
            {
                var transit = execute.WorkFlowDefinition.GetTransit(formStep.StepId, toStep.Key);
                if (transit != null)
                {
        ...
        private async Task<bool> ExecuteSubmitHandleTactic(WorkFlowTask currentTask, WorkFlowStep currentStep, WorkFlowExecution execute)
        {
        ...
        /// </summary>
        /// <returns></returns>
        private async Task<List<WorkFlowTask>> CreateNextStepTask(WorkFlowTask currentTask, WorkFlowStep currentStep, WorkFlowExecution execute)
        {
            ...
            foreach (var step in execute.Steps)
            {
                ...
                {
                    // 步骤如果不存在，直接忽略
                    var nextStep = execute.WorkFlowDefinition.GetStep(step.Key);
                    if (nextStep == null)
                    {
                        continue;
                    }
        ...
        /// <param name="execute"></param>
        /// <returns></returns>
        private async Task<bool> ExecuteBackBackTactic(WorkFlowTask currentTask, WorkFlowStep currentStep, WorkFlowExecution execute)
        {
            bool isNeedWating = false;
            ...
            else
            {
                ...
                {
                    var countersignatureStepPrevSteps = execute.WorkFlowDefinition.GetPrevSteps(currentStep.StepId);
        ...
        /// <param name="execute"></param>
        /// <returns></returns>
        private async Task<List<WorkFlowTask>> CreateBackStepTask(WorkFlowTask currentTask, WorkFlowStep currentStep, WorkFlowExecution execute)
        {
            ...
            var countersignatureStep = execute.WorkFlowDefinition.GetNextSteps(currentStep.StepId).Find(t => t.IsNeedCountersignature());
        ...
        /// </summary>
        /// <returns></returns>
        private async Task<WorkFlowTask> CreateFirstTask(WorkFlowExecution execute)
        {
            var firstStep = execute.WorkFlowDefinition.GetFirstStep();

            var task = new WorkFlowTask();

            task.Title = string.IsNullOrEmpty(execute.Title) ? "未命名任务(" + execute.WorkFlowDefinition.Name + ")" : execute.Title;
            task.FlowId = execute.FlowId;
        ...
        /// <param name="execute"></param>
        /// <returns></returns>
        private async Task CreateDuplicateTask(WorkFlowTask currentTask, WorkFlowExecution execute)
        {
            foreach (var step in execute.Steps)
            {
                var nextStep = execute.WorkFlowDefinition.GetStep(step.Key);
        ...
        /// </summary>
        /// <param name="execute"></param>
        private async Task<List<WorkFlowTask>> CreateTemporaryTask(WorkFlowTask currentTask, WorkFlowExecution execute)
        {
        ...
        /// </summary>
        /// <returns></returns>
        private async Task<bool> IsSubFlowStepPassed(WorkFlowTask currentTask, WorkFlowStep currentStep, WorkFlowExecution execute)
        {
            bool isPass = true;
        ...
        /// </summary>
        /// <returns></returns>
        private async Task<bool> IsCountersignatureStepPassed(WorkFlowTask currentTask, WorkFlowStep countersignatureStep, WorkFlowExecution execute)
        {
            //获取会签步骤的所有上一步
            var prevSteps = execute.WorkFlowDefinition.GetPrevSteps(countersignatureStep.StepId);
        ...
        /// </summary>
        /// <returns></returns>
        private async Task<bool> IsCountersignatureStepBacked(WorkFlowTask currentTask, WorkFlowStep countersignatureStep, WorkFlowExecution execute)
        {
            //获取会签步骤的所有上一步
            var prevSteps = execute.WorkFlowDefinition.GetPrevSteps(countersignatureStep.StepId);
        ...
        /// <param name="currentTask"></param>
        /// <returns></returns>
        private async Task<List<WorkFlowTask>> GetSameLevelStepNotExecuteTask(WorkFlowTask currentTask, WorkFlowExecution execute)
        {
            //获取下一步，反向推倒出
            var nextSteps = execute.WorkFlowDefinition.GetNextSteps(currentTask.StepId);

            var stepArray = new List<Guid>();

            foreach (var nextStep in nextSteps)
            {
                var nextStepPrevSteps = execute.WorkFlowDefinition.GetPrevSteps(nextSteps[0].StepId);
