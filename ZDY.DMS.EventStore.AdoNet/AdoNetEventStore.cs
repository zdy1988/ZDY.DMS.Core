...
namespace ZDY.DMS.EventStore.AdoNet
{
...
AND {this.GetEscapedFieldNames(propertyExpressions: x => x.OriginatorId)}={originatorIdParameterName}";
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    try
                    {
                        using (var command = connection.CreateCommand())
                        {
                            ...
                            var sortedDescriptors = descriptors.OrderBy(desc => desc.TimeStamp);
        ...
        protected EventDescriptor CreateFromReader(IDataReader reader)
        {
            ...
            return new EventDescriptor
            {
                ...
                TimeStamp = (DateTime)reader[this.config.GetFieldName(x => x.TimeStamp)],
